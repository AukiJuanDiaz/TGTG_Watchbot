...
def telegram_bot_sendtext(bot_message, only_to_admin=True):
    ...
    """
    send_text = 'https://api.telegram.org/bot' + bot_token + '/sendMessage?chat_id=' + str(config['telegram']["bot_chatID"]) + '&parse_mode=Markdown&text=' + bot_message
...
def telegram_bot_sendimage(image_url, image_caption=None):
    ...
    if image_caption != None:
        send_text += '&parse_mode=Markdown&caption=' + image_caption
    response = requests.get(send_text)
    return response.json()
...
def telegram_bot_delete_message(message_id):
    ...
    """
    send_text = 'https://api.telegram.org/bot' + bot_token + '/deleteMessage?chat_id=' + str(config['telegram']["bot_chatID"]) + '&message_id=' + message_id
    response = requests.get(send_text)
    return response.json()
...
def foodsi():
    ...
    for item in items:
        print(f"{item['store_name']}: {item['meals_left']}")
...
telegram_bot_sendtext("The bot script has started successfully. The bot checks every 1 minutes, if there is something new at TooGoodToGo or Foodsi. Every 24 hours, the bots sends a \"still alive\" message.")
refresh()
while True:
    # run_pending
